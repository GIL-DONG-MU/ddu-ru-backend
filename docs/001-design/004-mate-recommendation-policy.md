...
추천 결과 row는 당시 노출된 추천 스냅샷으로 유지합니다. 다만 홈 응답은 게시글의 현재 상태를 반영해야 합니다.

| 현재 게시글 상태 | 홈 추천 응답 정책 |
|---|---|
| `OPEN`, 정원 미마감, 여행 미종료 | 카드 노출, 신청 가능 |
| `CLOSED` | 카드 노출, 신청 불가 |
| 정원 마감 | 카드 노출, 신청 불가 |
| 여행 종료 | 카드 노출, 신청 불가 |
| soft deleted | 홈 추천 응답에서 숨김 |

#297 홈 응답 연결 시 추천 카드에는 현재 신청 가능 여부를 판단할 수 있는 응답 필드를 추가합니다. 정확한 필드명은 #297 API 응답 확장 작업에서 확정하되, 백엔드는 최소한 신청 가능/불가능 상태를 구분해 내려줘야 합니다.
...
- 추천 후 `CLOSED`, 정원 마감, 여행 종료 상태가 된 게시글은 카드에 포함하되 신청 불가로 내려줍니다.
- 추천 후 soft deleted 상태가 된 게시글은 홈 추천 응답에서 숨깁니다.
