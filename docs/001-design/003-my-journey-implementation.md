...
- **여행 날짜 수정**: 기본 정보 수정 API에서 `startDate` / `endDate`를 함께 입력하면 `posts` 테이블의 여행 날짜가 업데이트된다. 날짜는 항상 쌍으로 입력해야 하며(`startDate`만 또는 `endDate`만 입력 불가), `startDate`는 `endDate`보다 이전이어야 한다. 기간이 줄어드는 경우 새 기간 밖(`dayOffset >= 새 총 일수`)의 일정은 자동으로 소프트 삭제된다. 프론트에서 삭제 예정 일정이 있을 때 사전 안내 모달을 노출한다.
