...
- **갤러리 통합조회**: 여정 내 게시글 이미지를 최신 게시글 순, 같은 게시글 내에서는 `sort_order` 오름차순으로 통합 조회한다. 3열 격자 UI에 맞게 이미지 단위로 평탄화해서 반환하며, size는 이미지 수 기준이다. 커서는 `(journeyPostId, sortOrder)` 복합 커서를 사용한다. `post.id < cursorPostId OR (post.id = cursorPostId AND img.sortOrder > cursorSortOrder)` 조건으로 게시글 중간에서 잘려도 다음 페이지에서 정확히 이어받는다. 삭제된 게시글 이미지와 이미지 없는 게시글은 결과에서 제외된다.
