...
- 호스트는 게시글을 공지로 설정/해제하거나 상단에 고정할 수 있음
- 공지로 설정된 게시글은 홈 상단 우선 영역에 노출
- 나의 여정 홈에서는 가장 최근 공지 1건을 우선 노출
