...
> 사람이 합의해야 하는 제품 정책은 [홈 여행방 추천 정책](../002-policy/001-recommendation-policy.md)을 참고합니다.
...
|---|---|---:|---|
| 설문 미완료 또는 신청자 성향 없음 | 생성 안 함 | 0개 | `isAvailable=false`, `remainingFreeCount=0`, `recommendations=[]` |
| 추천 가능 사용자지만 후보가 0개 | `EMPTY` | 0개 | `isAvailable=true`, `remainingFreeCount=0`, `recommendations=[]` |
| 추천 가능 후보가 1개 | `COMPLETED` | 1개 | `isAvailable=true`, `remainingFreeCount=0`, 추천 1개 |
| 추천 가능 후보가 2개 | `COMPLETED` | 2개 | `isAvailable=true`, `remainingFreeCount=0`, 추천 2개 |
| 추천 가능 후보가 3개 이상 | `COMPLETED` | 상위 3개 | `isAvailable=true`, `remainingFreeCount=0`, 추천 3개 |
| 추천 저장 후 모든 추천이 마감/삭제 등으로 숨김 처리됨 | `COMPLETED` 유지 | 기존 row 유지 | `isAvailable=true`, `remainingFreeCount=0`, `recommendations=[]` |
